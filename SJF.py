...
def main():
    ...
    #sort the readyQueue
    def sort(input):
        for x in range(0,len(readyQueue)):
            readyQueue[x]=tuple(readyQueue[x])
        output=sorted(input)
        for x in range(0,len(readyQueue)):
            readyQueue[x]=list(readyQueue[x])
        return output
    totalTimer=0
    tLeft = 0
    cpuTimer=0
    ...
    while readyQueue:
        ...
        for x in range(0,len(readyQueue)):
            if readyQueue[x]!= []:
                pWaitingTime[readyQueue[x][1]]=pWaitingTime[readyQueue[x][1]]+currentBurst[0]
        for x in range(0,8):
